...
// Определите среднее время выполнения теста.
    ...
    val mapTestDuration = mapOf<String, Int>("the first" to 1000, "the second" to 300)
...
// о странице и статусе её проверки.
    ...
    resultsWebTesting.forEach { (url, status) ->
...
// "browserType". Ответ не может быть null.
    ...
    val config = mapOf("config01" to 1, "browserType" to 2)
...
// которых соответствуют определённой версии тестов.
    ...
    val scripts2 = scripts.containsKey("1_01")
    println(scripts2)
...
//Удалите из словаря с конфигурациями тестирования набор устаревших конфигураций.
        val configq = mapOf("config01" to 1, "browserType" to 2, "config03" to 3)

    val newConfig = configq - "config01"
    println(newConfig)
...
// (название можно получить вызвав фейковый метод, например getNameById(id: String))
    ...
    val testDuration = mapOf<String, Int>("ID1" to 500, "ID2" to 300, "ID3" to 5)
...
// условия тестирования.
    ...
    val perf = mapOf("version01" to 20, "version02" to 90, "version03" to 70)
    val perfImpr = perf.mapValues { it.value * 1.1 }
    println(perfImpr)

//Проверьте, пуст ли словарь с ошибками компиляции тестов.

    val сompilation = mapOf<String, String>()
    println(сompilation.isEmpty())
